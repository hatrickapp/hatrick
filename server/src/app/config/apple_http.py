...
async def verify_apple_id_token(client: httpx.AsyncClient, id_token: str, audience: str | None = None) -> dict:
    try:
    ...
    if header.get("alg") != "RS256" or not header.get("kid"):
        raise ValueError("Unsupported Apple identity token header.")

    response = await client.get(settings.apple.keys_url)
    ...
    now = int(time.time())
    expected_audience = audience or settings.apple.ios_bundle_id
    if claims.get("iss") != APPLE_ISSUER:
        raise ValueError("Invalid Apple identity token issuer.")
    if claims.get("aud") != expected_audience:
        raise ValueError("Invalid Apple identity token audience.")
