    ...
    async def __call__(self, scope, receive, send):
        ...
        if method != "OPTIONS" and not is_health_or_docs:
            ...
            if not app_sig or app_sig.decode("latin-1") != "XFzPIumBsq8aAyVMZX8Oq4GablUh6bDv+YwJl5qbedI=":
                log_msg = f"Access Denied! Path: {path}, Method: {method}. Received headers: { {k.decode('latin-1', errors='replace'): v.decode('latin-1', errors='replace') for k, v in raw_headers.items()} }"
