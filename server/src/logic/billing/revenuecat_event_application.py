...
async def handle_revenuecat_webhook(pool: Pool, event: RevenueCatEvent) -> None:
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            if user_id is not None and not await select_user_exists(conn, user_id):
            ...
            if not inserted or ignored is not None or user_id is None or event.app_user_id is None:
                return

            await apply_event(conn, user_id, event)


async def apply_event(conn: Connection, user_id: UUID, event: RevenueCatEvent) -> None:
    ...
    if event.event_type == "CANCELLATION":
        ...
        await demote_transferred_from_users(conn, event)
    ...
    await upsert_revenuecat_customer(
        conn,
        user_id=user_id,
        app_user_id=event.app_user_id or str(user_id),
    ...
    if event.event_type in ACTIVE_EVENT_TYPES or event.event_type in {"CANCELLATION", "TRANSFER"}:
        await update_user_plan(conn, user_id, "plus" if is_active else "free")
    elif event.event_type in DEMOTE_EVENT_TYPES:
        await update_user_plan(conn, user_id, "free")


async def demote_transferred_from_users(conn: Connection, event: RevenueCatEvent) -> None:
...
def ignored_reason(event: RevenueCatEvent, user_id: UUID | None) -> str | None:
    ...
    if user_id is None or not event.app_user_id:
